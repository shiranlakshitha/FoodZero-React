...
const PortfolioCard = ({ item }: { item: { name: string; meal: string; type: string; image: string } }) => {
  return (
    <div className="w-full text-white max-h-[600px] h-full overflow-hidden relative group hover: transition-all duration-300 cursor-pointer">
      ...
      <div className="z-10 absolute bottom-[15px] md:bottom-[30px] w-full px-[25px] flex flex-row justify-between gap-5 items-center">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-row gap-5 font-lato md:text-xl text-[12px]">
            <p>{item.meal}</p>
            <p className="cursor-pointer pointer-events-none">&#x2022;{item.type}</p>
          </div>
        </div>
